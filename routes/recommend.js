...
router.get('/recommend/add', function (req, res) {
    MongoClient.connect(DBURL, function (err, db) {
        db.collection('typeitem').find().toArray(function (err, recommendResult) {
            // console.log(typeResult)
            res.render('recommend/add', {
                recommendResult
            })
        })
    })
...
/***post提交实现类别添加 ok*/
router.post('/recommend/add', function (req, res) {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        MongoClient.connect(DBURL, function (err, db) {
            db.collection('recommenditem').insertOne(result, function (err, result) {
                // console.log(res)
                // 对拿到的数据进行判断
                if (err) {
                    // 无论成功失败都要返回页面
...
/***商品渲染****ok */
router.get('/recommend/index', function (req, res) {

    MongoClient.connect(DBURL, function (err, db) {

        db.collection('recommenditem').aggregate({
            $lookup: {
                from: 'typeitem',
                localField: 'typeid',
                foreignField: '_id',
...
/***删除商品 ok*/
router.get('/recommend/delete', function (req, res) {
    // res.send('删除')
    /**
     * 1-删除集合中的类别 (id)
     * 2-删除类别对应的图片(图片名称)
    ...
    MongoClient.connect(DBURL, function (err, db) {
        db.collection('recommenditem').deleteOne({
            _id
        }, function (err, result) {
            if (err) {
                // 无论成功失败都要返回页面
...
router.post('/recommend/edit', function (req, res) {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        MongoClient.connect(DBURL, function (err, db) {
            db.collection('recommenditem').update({
                _id
